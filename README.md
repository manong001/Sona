...
- 完整扫描会为歌曲生成轻量声学特征；“为你打造”按收藏歌曲的声学偏好推荐相似歌曲，每个合集保持单一语种，并排除已收藏和已隐藏内容。
- 随机列表按用户维护独立覆盖轮次：未抓取歌曲持续提高优先级，同时保留完播率权重，保证正常歌曲池最终全部出现。
- 管理员可只修改 Sona 数据库中的标题、艺人、专辑、曲号与曲风，并清除人工锁定后重新刮削，不回写原始音频。
- 管理员可通过服务端 OpenAI 兼容接口修复歌曲标题、识别主曲风和关联曲风，预览后确认写入；曲库可据此返回相似歌曲。
...
- 乐友圈提供公开昵称与签名、在线状态、好友搜索与双向关系、会话未读/已读、文本/表情/歌曲卡片消息和两分钟撤回。
- 朋友圈仅向本人及好友展示，支持图片、GIF、实况照片和视频，支持点赞、评论及本人/管理员删除；媒体保存在服务端数据目录。
...
```bash
SONA_AI_ENABLED=true
SONA_AI_BASE_URL=https://api.openai.com/v1
SONA_AI_API_KEY=你的密钥
SONA_AI_MODEL=服务支持的模型名称
```

服务器只会发送歌曲标题、艺人、专辑和当前曲风，不会上传音频文件。管理员可在“设置 → 歌曲分类 → 编辑元数据”中预览并应用 AI 建议。

管理员可在 App 的“设置 → 多源音乐下载”中搜索并加入下载队列。下载器只在 Docker 内网监听
`6700`，服务端通过 `SONA_SIDECAR_TOKEN` 鉴权；不要把该端口映射到公网。支持的默认来源为咪咕、网易云、QQ、酷我、千千、汽水和 Spotify，
...
生成仅包含 Intel `x86_64` 架构的 DMG：

```bash
./ios/build_x86_64_dmg.sh
```

默认产物写入 `ios/build/macos-x86_64/`。也可传入自定义输出路径：

```bash
./ios/build_x86_64_dmg.sh /自定义目录/Sona-x86_64.dmg
```

该脚本同样会执行 Release 构建、临时签名、DMG 创建与挂载验证，并确保最终应用只包含 `x86_64` 架构。
